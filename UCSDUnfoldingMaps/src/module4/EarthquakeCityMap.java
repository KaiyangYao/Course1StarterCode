...
//		earthquakesURL = "test1.atom";
	...
	private void addKey() {
		...
		// City Marker
		fill(color(148, 32, 34));
		triangle(55, 90, 50, 100, 60, 100);
...
//		ellipse(50, 225, 5, 5);
		...
		fill(0, 0, 0);
		text("City Marker", 75, 93);
		text("Land Quake", 75, 113);
		text("Ocean Quake", 75, 133);
		text("Deep", 75, 160);
		text("Intermediate", 75, 180);
		text("Shallow", 75, 200);
	...
	private void printQuakes()
	{
		...
		for(Marker cm : countryMarkers) {
			...
			for(Marker em : quakeMarkers) {
				...
				if((((EarthquakeMarker)em).isOnLand()) && (country.equals(countryName))) {
					count ++;
				}
			}
		...
		int oceanCount = 0;
		for(Marker em : quakeMarkers) {
			if(!((EarthquakeMarker)em).isOnLand()){
				oceanCount ++;
			}
		}
